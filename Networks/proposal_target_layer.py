...
def _unmap(data, count, inds, fill=0):
    ...
    if len(data.shape) == 1:
        ...
        ret[inds, :] = data
    return ret
def proposal_target_layer(rpn_cls_score,rpn_rois, gt_boxes,_num_classes):
    '''
    Make Python version of _proposal_target_layer_py below Tensorflow compatible
    '''    
    rois,labels,bbox_targets,bbox_inside_weights,bbox_outside_weights = tf.py_func(_proposal_target_layer_py,[rpn_cls_score,rpn_rois, gt_boxes,_num_classes],[tf.float32,tf.int32,tf.float32,tf.float32,tf.float32])
...
def _proposal_target_layer_py(rpn_cls_score,rpn_rois, gt_boxes,_num_classes):
    """
