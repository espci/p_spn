...
def test_net(data_directory, names, sess, tf_inputs, tf_outputs, max_per_image=300, key='valid', thresh=0.1, vis=False):
    ...
    all_boxes, detection_made = _detect_boxes(data_directory, names['EVAL'], sess, tf_inputs, tf_outputs, thresh, det_dir, key, vis)
...
def _detect_boxes(data_directory, names, sess, tf_inputs, tf_outputs, thresh, det_dir, key, vis):
    ...
    for i in tqdm(range(num_images)):
        ...
        # Read in file
        im_file = data_directory + '/Images/' + names[i]

        image = read_image(im_file)

        # Perform Detection
        boxes = _im_detect(sess, image, tf_inputs, tf_outputs)
...
def _im_detect(sess, image, tf_inputs, tf_outputs):
    ...
    image,img = image_preprocessing(image)

    im_dims = np.array(image.shape[1:3]).reshape([1, 2])

    feed_dict = {tf_inputs: image}

    # Evaluate the graph
    rois, bbox_deltas ,raw_output_up,preds_summary= sess.run(tf_outputs, feed_dict)
    ...
    temp = np.trunc(pred_boxes)
    temp = temp.astype(np.int)

    for i in range(len(boxes)):
        num = 0
        im = raw_output_up[temp[i][5]:temp[i][7],temp[i][4]:temp[i][6]]
        for j in range(im.shape[0]):
            for k in range(im.shape[1]):
                if im[j][k] ==11:
                    num+=1
        rate = num/(im.shape[0]*im.shape[1]+0.000000000000000001)
        if rate != 0:
            a=1
            #cv2.rectangle(img, (temp[i][4], temp[i][5]), (temp[i][6], temp[i][7]), (255, 0, 0), 2)
    plt.subplot(121)
    plt.imshow(preds_summary)
    plt.subplot(122)
    plt.imshow(img)
    plt.show()
