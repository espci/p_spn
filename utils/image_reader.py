...
def get_filename_list(path):
  ...
  for i in fd:
    i = i.strip().split(" ")
    i[0]='/home/qmy/label/image/'+i[0]
    image_filenames.append(i[0])

  return image_filenames

def cityscapeInputs(image_filenames, batch_size):

  images = ops.convert_to_tensor(image_filenames, dtype=dtypes.string)

  filename_queue = tf.train.slice_input_producer([images], shuffle=False)

  image,filename = cityscape_reader(filename_queue)
  reshaped_image = tf.cast(image, tf.float32)

  min_fraction_of_examples_in_queue = 0.4
  min_queue_examples = int(100 *
                           min_fraction_of_examples_in_queue)#367*0.4=146.8
  print ('wait.......' % min_queue_examples)

  # Generate a batch of images and labels by building up a queue of examples.
  return _generate_image_and_label_batch(reshaped_image,filename,
                                         min_queue_examples, batch_size,
                                         shuffle=False)

def cityscape_reader(filename_queue):
  ...
  image = tf.reshape(image_bytes, (1024, 2048, 3))

  return image,filename_queue[0]

def _generate_image_and_label_batch(image,filename, min_queue_examples,
                                    batch_size, shuffle):
