...
class ICNet_BN(Network):
    def __init__(self, image,cfg, mode,image_reader=None):
        self.cfg = cfg
        self.mode = mode
        ...
        if mode == 'train':
            ...
            super().__init__(inputs={'data': image}, cfg=self.cfg)
    ...
    def get_output_node(self):
        if self.mode == 'inference':
            ...
            self.logits = self.layers['conv6_cls']

            #logits_up = tf.image.resize_bilinear(logits, size=[1024,2048], align_corners=True)
            output = tf.argmax(self.logits, axis=3)
    ...
    def setup(self):
        ...
        (self.feed('conv3_1_1x1_proj_bn',
             ...
             .interp(s_factor=0.5, name='conv3_1_sub4') # 这里是在feature的基础上直接降采样
        ...
        self.conv5_3 = self.layers['conv5_3/relu']
